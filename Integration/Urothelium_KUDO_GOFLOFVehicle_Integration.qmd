---
title: "Urothelium_KUDO_GOFLOFVehicle_Integration"
author: "Xin Wang"
date: "2026-08-05"
...
Description:
  Integrate only the three KUDO urothelium organoid conditions of interest (GOF, LOF, Vehicle;
  the Yoda condition is excluded). This follows the same pipeline as Preprocess/KUDO_Integration.rmd
  (per-sample QC -> DoubletFinder -> merge -> Harmony integration -> clustering), run independently on
  GOF/LOF/Vehicle so that doublet calling and clustering are not influenced by the Yoda sample.

  Downstream of the integration this notebook adds:
   1. QC plots restricted to GOF/LOF/Vehicle
   2. Per-condition dot plots of the urothelium lineage marker genes
   3. Bar plot of kidney (non-urothelial nephron segment / immune) marker genes to confirm organoid purity
   4. Upk+ vs Krt5+ lineage proportion analysis (sorted) across conditions
   5. Notch signaling pathway profiling using the MSigDB human gene set WP_NOTCH_SIGNALING_PATHWAY
      (WikiPathways, C2:CP:WIKIPATHWAYS), converted to mouse orthologs with nichenetr
   6. GSEA of cell proliferation and cell differentiation gene sets (MSigDB GO:BP) for the three
      pairwise contrasts GOF vs Vehicle, LOF vs Vehicle, GOF vs LOF

Dataset Description:
  KUDO mouse urothelial organoid scRNA-seq (PIPseeker v3.3, GRCm39), three conditions:
    Vehicle - control organoid
    GOF     - Notch gain-of-function organoid
    LOF     - Notch loss-of-function organoid
  Reference for the organoid system: Santos, C.P., Lapi, E., Martinez de Villarreal, J. et al.
  Urothelial organoids originating from Cd49fhigh mouse stem cells display Notch-dependent
  differentiation capacity. Nat Commun 10, 4407 (2019). https://doi.org/10.1038/s41467-019-12307-1
---
...
## Step 3: Per-sample QC and preprocessing (same as KUDO_Integration.rmd, restricted to GOF/LOF/Vehicle)
```{r}
scrna.list <- list()
...
## Step 4: Merge GOF/LOF/Vehicle and run QC plots
```{r}
setwd(QCdir)
...
## Step 5: DoubletFinder per condition (same function as KUDO_Integration.rmd)
```{r}
setwd(DoubletDir)
...
## Step 6: Harmony integration and clustering (same as KUDO_Integration.rmd)
```{r}
setwd(IntegratedDir)
...
## Step 7 (task item 3): Dot plot of urothelium marker genes for each condition
```{r}
...
## Step 8 (task item 4): Bar plot of kidney (non-urothelial) marker genes
```{r}
...
## Step 9 (task item 5): Upk+ / Krt5+ lineage proportions across conditions (sorted)
```{r}
setwd(LineageDir)
...
## Step 10 (task item 6): Notch signaling pathway profiling (MSigDB WP_NOTCH_SIGNALING_PATHWAY)
```{r}
setwd(NotchDir)
...
## Step 11 (task item 7): GSEA of cell proliferation and cell differentiation (pairwise contrasts)
```{r}
setwd(GSEADir)
