...
##########################################################################################
# method 2 : we also can do the SCTransform for each sample, and then merge them together, and then run the harmony for the batch correction
##########################################################################################
# Reference: https://davetang.github.io/muse/harmony.html#Seurat_workflow_with_harmony
...
cell_by_cond <- split(row.names(scrna@meta.data), scrna@meta.data$Sample)

vfeatures <- lapply(cell_by_cond, function(cells){
  FindVariableFeatures(object = scrna[, cells], selection.method = "vst", nfeatures = 2000) |>
    VariableFeatures()
})
...
# |>
#   RunPCA(features = VariableFeatures(scrna), npcs = 20, verbose = FALSE)
# scale and perform PCA on the merged object
# Step 4: PCA + Harmony (optional)
scrna <- RunPCA(scrna, features =VariableFeatures(scrna),npcs=20)

scrna <- RunHarmony(scrna, group.by.vars = 'Sample', dims.use = 1:20, plot_convergence= FALSE, reduction = "pca", reduction.save = "harmony")
...
saveRDS(scrna, file = "KudoUUOComparision_Scrna_Merged_RunHarmony.rds")
